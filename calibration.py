...
class StereoCalibration:
    ...
    def create_chessboard_points(self) -> None:
        ...
        for left_image_path, right_image_path in tqdm(
            ...
            if self.corners_path is not None:
                ...
                base, ext = os.path.splitext(left_image_name)
                left_image_name = f"{base}_corners{ext}"
                right_image_name = os.path.basename(right_image_path)
                base, ext = os.path.splitext(right_image_name)
                right_image_name = f"{base}_corners{ext}"
        ...
        print("코너 데이터를 .npy 파일로 저장합니다...")
        # list를 numpy array로 변환 시, 각 원소의 shape이 다를 수 있으므로 dtype=object를 사용합니다.
        np.save(os.path.join(self.input_path, 'corners_left.npy'), np.array(self._chessboard_2d_left_points, dtype=object))
        np.save(os.path.join(self.input_path, 'corners_right.npy'), np.array(self._chessboard_2d_right_points, dtype=object))
        print("저장 완료: corners_left.npy, corners_right.npy")
    ...
    def rectify_calibration_images(self) -> None:
        ...
        for left_image_path, right_image_path in tqdm(
            ...
            left_image_name = os.path.basename(left_image_path)
            base, ext = os.path.splitext(left_image_name)
            left_image_name = f"{base}_rectified{ext}"
            right_image_name = os.path.basename(right_image_path)
            base, ext = os.path.splitext(right_image_name)
            right_image_name = f"{base}_rectified{ext}"
            left_image_path = os.path.join(
